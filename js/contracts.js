...
function initHistoryFilters() {
  ...
  buildSel.innerHTML = '<option value="">전체 건물</option>' +
    props.map(p => `<option value="${p.name}">${p.name}</option>`).join('');
  buildSel.value = ''; // 기본: 전체 (빈값=전체)
  ...
  renderContractHistory(); // 초기 전체 표시
}
...
function renderContractHistory() {
  ...
  const activeRecords = activeFiltered.map(t => {
    ...
    return {
      ...
      name: t.name, phone: t.phone, type: t.type || '월세',
  ...
  function isExt(rec) {
    ...
    if (rec.reason === 'eviction' || rec.reason === 'manual') return false;
    return allRecords.some(r =>
      r !== rec && r.building === rec.building && String(r.room) === String(rec.room) &&
      r.name === rec.name && r.phone && rec.phone && r.phone === rec.phone &&
  ...
  tbody.innerHTML = allRecords.map(r => {
    ...
    if (r._isCurrent) {
      ...
      statusBadge = `<span class="badge badge-gray">퇴거</span>`;
      rowStyle = '';
    }
    ...
    return `<tr style="${rowStyle}">
      ...
      <td>${r.name||'-'}</td>
      <td>${leaseTypeBadge ? leaseTypeBadge(r.type, r.deposit) : (r.type||'-')}</td>
...
function onHaBuildingChange() {
  ...
  dongSel.disabled = true; roomSel.disabled = true;
  if (!building) return;
...
function saveHistoryAdd() {
  ...
  const newRec = {
    id: uid(), building, dong, room: roomNo, name,
...
function deleteHistoryRecord(id) {
  ...
  if (!rec) return;
  confirm(
    `'${rec.name}' 계약 이력(${fmtDate(rec.contractStart)} ~ ${fmtDate(rec.contractEnd)})을 삭제하시겠습니까?`,
    () => {
...
function archiveTenant(tenantId, extraNote) {
  ...
  history.push({
    ...
    building: t.building, dong, room: t.room,
    name: t.name, phone: t.phone, type: t.type || '월세',
    deposit: t.deposit, rent: t.rent, management: t.management,
  ...
  if (idx >= 0) {
    tenants[idx] = {
      ...
      type: '월세', status: '공실', note: '', extendCount: 0
    };
